...
\minihead{Request handling semantics.}
Request/response follows a one-request/one-response rule at the transport level: the receiver returns \type{ResponseData} from \type{IModule::processRequest}, and the core (or hosting wrapper) constructs and sends exactly one response back to the requester while preserving \type{MessageHeader.id_} (see \autoref{subsec:ch3_comm_primitives} and \autoref{subsec:ch3_message_lifecycle}).

\minihead{Rules for module implementers.}
\begin{itemize}
    \item \textbf{Restricted core usage during construction and teardown.}
    During module creation/removal (constructor, destructor, \type{threadStart}, \type{threadStop}), module code must not call any core methods that depend on a stable module graph: in particular \type{ICore::sendMessage}, \type{ICore::sendRequest}, \type{ICore::sendResponse}, or any \type{ICoreControl} reflection/orchestration queries.
    The only allowed core interaction in these phases is shared-payload allocation management, i.e., \type{ICore::createDynamicAllocator}, \type{ICore::createBufferAllocator}, and \type{ICore::deleteAllocator} (and logging via the provided \type{logging::ILogger}).
    Message sending and \type{ICoreControl} interaction should be treated as valid only during the module's \emph{active running phase}. In particular, module code must not rely on being able to send messages or query core state during construction/startup (\type{ctor}/\type{threadStart}) or during teardown (\type{threadStop}/\type{dtor}): a compliant core may block these calls (e.g., due to module-graph locks) or discard them as part of removal.
...
Some wrappers and higher-level infrastructure may require optional module capabilities beyond \type{IModule}. For this purpose, modules may expose additional interfaces through a capability query mechanism (e.g., \type{query_capability(typeid(T))}). This keeps the base contract minimal while enabling optional features; concrete usage is described later where wrappers are introduced.
...
\textbf{Construction inputs and persistence.}
...
\textbf{Message sending helpers.}
\type{BaseModule} provides thin wrappers around the three core send functions:
\begin{itemize}
    \item \type{sendMessage(publish_producer_id, message)} fills \type{MessageHeader.timestamp_ns_} and sets \type{MessageHeader.success_ = true} (the \type{success_} flag is only meaningful for responses), then calls \type{ICore::sendMessage} with a \type{ChannelIdentifier} constructed from the stored \type{module_id} and the provided producer channel index.
