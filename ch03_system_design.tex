...
\subsection{Roles and separation of responsibilities}
...
To allow modules to communicate, typically we would need a publish/subscribe infrastructure with some identifier to specify the type of message being sent. We extend this to also support request/response communication (for example, publish/subscribe is great for streaming camera data, but for scene detection, we want to send a request to the scene detection module and get a response with the detected objects so that we do not need to detect the scene on every frame). Therefore we have a publish/subscribe and request/response model, where modules can declare their inputs (consumers; subscribe or request) and outputs (producers; publish or response) of both types. Each of these inputs and outputs is associated with a channel type identifier (a string) that specifies the type of message being sent. This approach is visualized in \autoref{fig:ch3_channel_mapping}.
...
Finally the auto-create flag allows a module to be automatically created when the system starts, without the need for manual instantiation. This is useful for modules that are essential for the system to function, such as the UI module. Auto-created cannot require any specific channel mapping, since they are created automatically. They can still have consumer channels, but the channels have to be of a specific type as we outline in \autoref{subsec:ch3_channel_contract}.
...
We need to define the message queue capacity to avoid unbounded memory growth in case a producer produces messages faster than the consumer can process them. The system also supports modules deciding what to do when the message queue is full, this will be covered in \autoref{subsec:ch3_message_lifecycle}.
