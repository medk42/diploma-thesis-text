...
In this section, we describe the module contract, which defines the core abstractions and communication patterns that all modules and the runtime core must follow. The contract is independent of any specific implementation of the core or modules, and it provides a stable foundation for building the system.

\subsection{Roles and separation of responsibilities} \label{subsec:ch3_roles}

The contract defines two main interfaces that separate the responsibilities of modules and the core: \type{IModule} and \type{ICore}. Every module implements \type{IModule}, which defines its lifecycle, how it handles incoming messages, serialization of module state, and a few other required methods. The core exposes \type{ICore}, which provides services to modules, including message routing, shared-memory allocation for large payloads, and system management capabilities (module discovery, instantiation, save/load, etc.).

As shown in \autoref{fig:ch3_four_parts}, modules do not communicate directly. Instead, all interaction goes through the core, which routes messages between modules according to an explicit channel mapping. When a module wants to publish data or send a request/response, it calls the corresponding method on \type{ICore}. The core then delivers the message to the mapped recipient(s). On the receiving side, the core enqueues the message for processing and invokes the appropriate \type{IModule} callback from a core-managed worker thread. The contract therefore assumes concurrent execution: multiple worker threads may call into the same module instance at the same time (e.g., processing different channels, or overlapping message and request handling). Modules must treat these callbacks as multi-threaded entry points and implement the necessary synchronization for their internal state. The concrete queueing and worker configuration is specified as part of the module declaration and described next.
...
To allow modules to communicate, the contract defines two complementary communication primitives: publish/subscribe for streams and request/response for on-demand queries. Publish/subscribe is used for continuously produced data such as camera frames, while request/response is used when the producer should run only when asked (e.g., a scene module performs detection only after receiving a request and returns the detected objects). Modules therefore declare their inputs (consumers: subscribe or request) and outputs (producers: publish or response). Each channel is tagged by a channel type identifier (a string) that specifies the message type. An example mapping is shown in \autoref{fig:ch3_channel_mapping}.

Each module then has to contain a \type{ModuleInfo} structure that contains the information about the module. This structure contains the following fields:
\begin{itemize}
    \item module type identifier (a string that uniquely identifies the type of the module, for example \type{CameraModule} or \type{SceneDetectionModule}),
    \item module name (a user-friendly name of the module, for example ``My Camera Module'' or ``My Scene Detection Module''),
    \item module description (a user-friendly description of the module, for example ``This module captures images from a camera and publishes them on the camera channel'' or ``This module detects objects in the scene and publishes them on the scene channel''),
    \item list of publish channels (module produces camera stream),
    \item list of response channels (module accepts scene detection requests and produces scene detection responses),
    \item list of subscribe channels (module consumes camera stream),
    \item list of request channels (module sends scene detection requests and consumes scene detection responses),
    \item auto-create flag (described later),
    \item worker thread count (described later).
\end{itemize}

The module uses the lists of channels to define its input requirements and output capabilities. In our system, the mapping between publish/subscribe and request/response channels is not implemented as a broadcast, but rather as a directed connection between specific producer and consumer channels --- the type of the channel just specifies the message type, however the actual wiring between existing producer channels from already initialized modules and the consumer channels of the module being instantiated is defined during module instantiation by providing a channel mapping (in \autoref{fig:ch3_channel_mapping} we can see that the \type{Pen} module receives \type{camera_stream} messages only from \type{Camera A}). The channel mapping defines, for each consumer channel of the module being instantiated, which producer channel(s) it is connected to --- by identifying the producer module and the producer channel for each bound connection. This is done to allow for more flexibility in the system, for example (as seen in \autoref{fig:ch3_channel_mapping}), if we have two camera modules that produce camera stream, we can choose which one to connect to the pen tracking module. 

As mentioned before in \autoref{subsec:ch3_roles}, the core handles dispatching worker threads to process incoming messages for modules. The module can specify how many worker threads it needs, which allows it to process multiple messages in parallel. The worker threads are initialized per module and there are two types: \emph{prioritized} and \emph{regular}. 

Prioritized worker threads are dispatched to process messages from channels that are marked as prioritized, while regular worker threads are dispatched to process messages from non-prioritized channels. The multiple worker threads and the prioritization mechanism are both crucial for fast UI responsiveness, as they allow modules to process UI messages in parallel with other messages that require heavy processing (camera data for pen tracking). 
...
In the previous section, we described the module contract, which defines how modules declare their inputs and outputs in terms of channels. In this section, we describe how channels are defined and how the channel mapping works in more detail.

There are two types of channels: producer channels (covered by \type{communication_channel::Producer} structure) and consumer channels (covered by \type{communication_channel::Consumer} structure). 

The \type{Producer} structure contains the following fields:
\begin{itemize}
    \item channel type identifier (a string that uniquely identifies the type of the channel, for example \type{camera_stream} or \type{scene_req_resp}),
    \item channel name (a user-friendly name of the channel, for example ``Camera Stream'' or ``Scene Detection Response'' --- this is used for visualization purposes in the UI to show the user what channels are available and what they represent),
    \item channel description (a user-friendly description of the channel, for example ``This channel publishes images from a camera'' or ``This channel publishes the response to scene detection requests''),
    \item prioritized flag (a boolean that indicates whether the channel is prioritized or not),
    \item message queue capacity (an integer that specifies the maximum number of messages that can be queued for this channel)
\end{itemize}

Prioritization in the producer is only used for response producers, since request-response covers sending messages both ways, and if the producer is prioritized, the request is processed by a prioritized worker thread.

We define a message queue capacity to bound the amount of pending work at the receiver. If incoming messages/requests arrive faster than the module can process them, the queue capacity prevents unbounded buffering; the module can then decide how to handle overload (drop, replace, etc.), described in \autoref{subsec:ch3_message_lifecycle}.

The \type{Consumer} structure contains the following fields:
\begin{itemize}
    \item channel type identifier, channel name, channel description, prioritized flag and message queue capacity (same as in the producer),
    \item count (single, range or auto-all; specifies how many producer channels of the same type can be connected to this consumer channel; single means that only one producer channel of the same type can be connected, range means that a specific range of producer channels of the same type can be connected, auto-all means that all producer channels of the same type will be automatically connected to this consumer channel),
    \item min and max (integers that specify the minimum and maximum number of producer channels of the same type that can be connected to this consumer channel; only used if count is range).
\end{itemize}

Prioritization in the consumer is used for both subscribe and request channels. For subscribe channels, if the channel is prioritized, the messages from that channel are processed by a prioritized worker thread. For request channels, if the channel is prioritized, the response is processed by a prioritized worker thread.

Consumer contains the count field which allows for more flexibility in the channel mapping. For example, if we have a scene detection module that can accept input from multiple camera modules, we can set the count to a range of 2 to 4 (see \autoref{fig:ch3_channel_mapping}). This means that the scene detection module can be connected to at least 2 and at most 4 camera modules. If we set the count to auto-all, it means that the scene detection module will be automatically connected to all camera modules that produce camera stream. This is required for the auto-created modules, since they cannot receive a specific channel mapping, but they can still have consumer channels that are automatically connected to all producer channels of the same type. For example, the UI module is auto-created and it has a subscribe channel for camera stream that is set to auto-all, which means that the UI will be automatically connected to all camera modules that produce camera stream and so it will receive all the messages from those channels. This combines the advantages of fixed channel-to-channel mapping and full broadcast. 


\subsection{Message structure: header and optional shared blobs}

The message structure is designed to be efficient for both small control messages and large data payloads. Messages structure is defined in the \type{MessageHeader} structure. This header is used both on the producer side (when sending a message) and on the consumer side (when receiving a message). The header contains the following fields:
\begin{itemize}
    \item data pointer and length (this covers the ``header'' of the message and is copied from producer to each consumer; this is enough for most messages, for example pen intent, control messages, etc.),
    \item array of blobs (via blob pointer and length; this is used for large data payloads, for example camera frames, scene data, etc.)
    \item id and success flag (only used for request/response messages; the id is used to match requests and responses, while the success flag indicates whether a failure occurred on a transport level; for example wrong channel mapping or unknown request in the receiving module)
    \item timestamp
\end{itemize}

The data pointer and length cover the main content of the message, which is typically small and can be copied efficiently. For larger payloads, such as images or point clouds, the message can include an array of blobs. Blobs are designed to avoid repeated copying of large data between modules --- for example, a camera module can capture an image and allocate a blob in shared memory to hold the image data, then it can publish a message with a pointer to that blob. The consumer module (or multiple modules) can then access the blob directly without copying the image data and even forward it as a part of its own message to other modules (camera module -> calibration module -> pen tracking module). Blobs are represented by the \type{SharedDataBlob} class, which contains a reference to the core-allocated data (\type{ISharedData*}) and handles reference counting and deletion of the data. When all modules are done with the blob, it will be automatically deallocated. This design allows for efficient sharing of large data between modules while keeping the main message header small for efficient routing and processing of control messages.

Allocation of \type{SharedDataBlob} is done through \type{IAllocator} objects. Modules can request an allocator from the core via \type{ICore::createDynamicAllocator()} or \type{ICore::createBufferAllocator(slot_size, slot_count)}. The module can then use the allocator to create blobs of the desired size. The allocated blobs are automatically registered with the core and can be shared between modules. The producer module is responsible for allocating the blob and including a pointer to it in the message header, while the consumer modules can access the blob through the pointer in the message header. The blob allocators and therefore the blobs themselves must be handled by the core, because the core defines the runtime environment (e.g., threads or processes) and therefore it is the only component that can ensure that the shared memory is properly managed and accessible to all modules. Modules are required to remove the requested allocators when they are done with them (via \type{ICore::deleteAllocator(allocator)}) to avoid memory leaks --- this is already handled by the \type{BaseModule} class that all modules inherit from (described in \autoref{subsec:ch3_basemodule}), so module implementers do not have to worry about it.
