...
In this section, we describe the module contract, which defines the core abstractions and communication patterns that all modules and the runtime core must follow. This contract is independent of any specific implementation of the core or modules, and it provides a stable foundation for building the system.

\subsection{Roles and separation of responsibilities} \label{subsec:ch3_roles}

The contract defines two main interfaces that separate the responsibilities of modules and the core: \type{IModule} and \type{ICore}. Every module must implement the \type{IModule} interface, which defines its lifecycle, how it handles incoming messages, serialization of the module state and few other important methods. The \type{ICore} interface then defines the services that the core provides to modules, such as message routing, memory management for large payloads and system management capabilities (module discovery, instantiation, save/load functionality, etc).

As seen on \autoref{fig:ch3_four_parts}, there is no direct module-to-module communication. Instead, all interaction goes through the core, which routes messages between modules based on the declared channel mapping. When a module wants to send a message, it calls the corresponding method on the \type{ICore} interface. The core then delivers the message to the appropriate recipient modules based on a channel mapping. On the target module's side, the core then enqueues the message for processing, and when possible dispatches a worker thread to call the target module's message processing method on the \type{IModule} interface.
