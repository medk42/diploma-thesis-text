...
Activate the robot module by selecting \emph{Kassow Robot Module} \uicall[fig:ch5_activate_robot]{2}, setting the \emph{Host} parameter \uicall[fig:ch5_activate_robot]{3} to the robot controller IP address, selecting the correct \emph{Robot Model} \uicall[fig:ch5_activate_robot]{4} for your hardware, and pressing \emph{Activate} \uicall[fig:ch5_activate_robot]{5}. Wait for the activation procedure to finish: the \emph{Activating Module} pop-up message will disappear, \emph{Activate} button will become disabled, and the module status indicator will turn green to indicate the module is ready.

If activation fails, verify the network connection to the robot controller and re-check the IP address. Robot model parameter is purely for visualization and does not affect the robot behavior.
...
After collecting samples, press \emph{Activate} \uicall[fig:ch5_activate_calibration]{4} to run calibration. If calibration fails, the captured dataset likely did not contain enough sharp, well-lit, diverse stereo views of the board (simultaneously visible in both cameras) or did not contain enough robot motion diversity; capture more samples with larger board coverage and stronger pose variation and retry.

You can switch to the main (\emph{Aergo}) view by clicking \emph{Close} \uicall[fig:ch5_activate_calibration]{5} to see the live camera preview to help with capturing good calibration samples; the preview shows only one camera's view.
A set value can be cleared using the ``$\times$'' button inside its value field \uicall[fig:ch5_activate_calibration]{6}.
A list entry can be removed using the ``$\times$'' button next to the entry \uicall[fig:ch5_activate_calibration]{7}.
The list enforces a minimum size of 10 entries; entries cannot be removed below this limit.
...
The save dialog suggests a unique name. You can change the name before saving. If you try to save with a name that already exists, the dialog will ask for confirmation to overwrite the existing save. 

If you save the setup with robot and camera modules activated, the modules will attempt to activate when loading the setup later. If that activation fails (e.g., robot or camera not available), the loading procedure will fail and the module graph will not be restored. You may want to save the setup with the robot and camera modules in a deactivated state to avoid this potential loading failure. The calibration module can remain activated.
