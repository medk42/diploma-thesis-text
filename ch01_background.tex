...
Beyond AR-only interaction, there is a growing interest in combining gestures (e.g. pointing) with natural language to express intent at a higher semantic level than explicit waypoints. Lin et al.\ propose \emph{GIRAF}, a framework that combines gesture recognition with large language model-based reasoning to interpret multimodal instructions in a context-dependent manner, and demonstrate it on several tabletop manipulation tasks \cite{Lin2023}. Conceptually, this direction is promising, as it removes the need for explicit interaction with flat-screen or AR-based interfaces and instead enables direct natural language communication with the system.

That being said, the presented instantiation of \emph{GIRAF} is evaluated in a narrowly scoped tabletop manipulation setting and relies on a collection of task- and domain-specific perception, grounding, and reasoning components. Extending the system to other tasks or environments would require substantial additional development. Additionally, the system is used for real-time \emph{control} of the robot, rather than programming. In contrast, our thesis focuses on an extensible system for \emph{programming} a wider range of tasks with shared sensing and authoring capabilities. However, multimodal intent understanding (e.g., gesture and speech) is a promising direction for future work to further reduce friction in robot programming, and we discuss it further in \autoref{chapter:ch7_discussion}.


\section{Positioning of this thesis}

In summary, existing work shows multiple viable ways to reduce programming friction: AR-assisted online waypoint/path authoring with validation \cite{Ong2020}, rule-oriented online authoring in AR \cite{Ikeda2024}, and offline digital twin AR workflow that allows online-like programming without robot downtime \cite{Muller2024}. Hand-tracking-based waypoint authoring is also a promising direction for reducing friction, but it may require additional work to achieve the required accuracy for precise pose specification \cite{Vysocky2023}. Multimodal approaches such as \emph{GIRAF} show promise for enabling more natural communication of intent, but they are still in early stages of development and evaluation \cite{Lin2023}.

Compared to these approaches, our thesis targets an extensible task-oriented online spatial authoring system that attempts to avoid the need for users to specify low-level motion details by inferring them from spatial intent and scene context by using reusable sensing, authoring and execution pipelines.