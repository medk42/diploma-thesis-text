...
\xxx{State exactly what folders are included in the submitted code archive (matches \autoref{appendix:source_code}): \path{diploma-thesis-code/backend/}, \path{diploma-thesis-code/default_modules/}, \path{diploma-thesis-code/manifests/}, \path{diploma-thesis-code/vcpkg-triplets/}; plus root-level artifacts as listed in \autoref{appendix:source_code}.}
\xxx{State what is excluded at a high level (development-only/non-essential repository content); detailed packaging and build notes are in \autoref{appendix:source_code}.}
\xxx{Point to Appendix for external artifacts: pen CAD + firmware and assembly notes (\autoref{appendix:pen_artifacts}), Charuco board PDF and exact dimensions (\autoref{appendix:charuco}), scene registry and example CAD (\autoref{appendix:scene_registry}, \autoref{appendix:scene_cad}), and robot-side component (CBun) sources/build notes (\autoref{appendix:cbun}).}

\minihead{Implementation note.}
\xxx{State: implemented in C++ (verify from repository structure / toolchain; build notes in \autoref{appendix:source_code}).}
\xxx{State: designed to run on Windows and Linux; most logic is platform-neutral. Platform-specific code is primarily in camera acquisition (\autoref{subsec:ch4_mod_stereo_camera}) and the chosen UI/web stack; macOS not validated. (Verify which modules are built/used on Linux in your build system.)}
...
The concrete artifact files (CAD, firmware, PDFs, registry JSON) shipped with the thesis attachment are listed in \autoref{appendix:artifacts}.
...
\xxx{Reference the mount CAD artifact in \autoref{appendix:camera_mount_cad} and the mount render in \autoref{fig:ch4_camera_mount}.}
...
\xxx{Note: CBun sources and build notes are provided in \autoref{appendix:cbun}; operator installation/enabling is described in \autoref{chapter:ch5_user_workflow}.}
...
\xxx{Reference the concrete CAD/firmware artifacts and reproducibility notes in \autoref{appendix:pen_artifacts} (including the internal layout render and assembly notes).}
\xxx{Briefly mention D-Point license attribution boundary as documented in the appendix (see \autoref{appendix:pen_artifacts}).}
...
\xxx{Reference the registry and example CAD artifacts in \autoref{appendix:scene_registry} and \autoref{appendix:scene_cad}.}
\xxx{Reference \autoref{fig:ch4_scene_box}.}
...
\xxx{Add one referencing sentence: "An example box object used in the prototype is illustrated in \autoref{fig:ch4_scene_box}."}
...
\xxx{Reference the provided PDF and exact dimensions in \autoref{appendix:charuco} (do not re-list the numeric parameters here unless needed for the narrative).}
\xxx{Scene objects are configured via a JSON registry (see \autoref{appendix:scene_registry}); refer back to \autoref{subsec:ch4_hw_scene} for the physical objects.}
