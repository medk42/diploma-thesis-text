...
Based on requirement \ref{req:scene_detection_logic}, our system needs to provide \emph{scene detection} logic that can detect objects in the scene and provide their poses. The accuracy of this detection matters much more than in the 6D input interface, because we use the detected objects to refine the user input. For example, in the seam welding use case, we rely on the detected objects to find the seam between them and align the user-defined trajectory to it --- if the detected object poses are inaccurate, the seam detection and trajectory alignment will also be inaccurate, which will directly impact execution quality.

When selecting an approach for scene detection, we consider:
\begin{enumerate*}[label=(\roman*)]
    \item accuracy/robustness,
    \item setup effort,
...
\begin{enumerate*}[label=(\roman*)]
    ...
    \item \emph{single-view model-based pose estimation}, where the system detects objects from a single camera based on known 3D models,
    \item \emph{multi-view model-based pose estimation}, where the system detects objects from multiple cameras based on known 3D models,
    \item \emph{3D scanning and reconstruction}, where the system reconstructs a 3D model of the scene from many cameras and detects objects based on it,
    \item \emph{depth-based 3D scanning and reconstruction}, where the system merges multiple depth images of the scene into a 3D model and detects objects based on it, and
    \item \emph{commercial systems} that handle scene detection.
\end{enumerate*}

\xxx{well, let's consider them...}

\xxx{In the current system, scene objects are restricted to primitive geometric proxies (e.g., boxes with known dimensions). This is sufficient for the demonstrated tasks but limits generality.}

We treat fiducial-based scene detection as a controlled proxy that isolates the interaction and execution aspects of spatial authoring, while keeping the sensing component replaceable.

\xxx{In the long term, accurate scene understanding is central because it provides the reference frames that ground and refine user-authored trajectories. In this thesis, however, we prioritize validating the spatial authoring workflow end-to-end on a real robot under time and engineering constraints. We therefore implement a simplified scene detection approach based on fiducial markers attached to relevant objects. This choice trades generality for reliability and fast iteration: it provides stable object poses for the demonstrator while keeping the sensing stack lightweight. The limitation is that objects must be instrumented and the approach does not generalize to arbitrary unprepared scenes; we discuss this explicitly in Chapter~7 and outline how model-based 6D pose estimation (e.g., BOP-style pipelines) could replace the fiducial module in future work.}
