...
- Conclusion
    - Test results/experiments - or move this to the analysis? We could move this along with the pen HW/FW/SW and scene detection implementation to the analysis - as part of the chosen solution, after the decomposition section.
        - Tracking stability / repeatability
        ...
        - End-to-end workflow metrics (fast, strong narrative), for 2-3 representative tasks:
            - number of user actions (clicks/holds/steps)
            - time-to-define-trajectory (rough, even if only you as operator)
            - failure modes observed (lost tracking, wrong object pose, UI confusion) - what specifically would we put here? Both on teach pendant and my app, or what happens in my app that could cause issues for the user? If so, should we separate it to issues that are because of the demo and issues that are inherent to this design? I think that this maybe shouldnt be just observed, but also real limitations from both the use and "imagined/analyzed". But that fits better in the "what we achived" section.
        - Real robot validation: what was tested, what wasn’t, why
        - Qualitative “what we learned” - this should be in what we achived / weaknesses
            - where spatial authoring shines (welding seam definition / fast point placement) - again, more into the what we achived section, here we're mixing the two a bit
            - where it struggles (scene detection precision; complex program logic; UI overhead) - again, overlap, also with "Known weaknesses"
