...
This appendix summarizes the submitted electronic attachment package: the pruned source code snapshot used to build the implemented system, the external build/dependency boundaries, and the hardware/calibration artifacts required to reproduce the physical setup.
...
The electronic attachment submitted with this work contains a pruned snapshot intended for building and inspection; it may not correspond to a single stable tagged revision of the development repository.
...
This work distributes the implementation as source code only (no third-party binaries).
...
The firmware is derived from the upstream D-Point project (Jcparkyn/dpoint) and modified for this system; the upstream license text is included under \path{third_party_licenses/}.
The mechanical design is original work, with high-level inspiration taken from D-Point.
...
An example registry instance used in this work is:
...
Render of this example box is shown in \autoref{fig:ch4_scene_box}.

\subsection{Robot-side component (CBun) and reproducibility boundary}
\label{appendix:cbun}

The system uses a robot-side component (CBun) that runs on the Kassow controller and exposes the robot capabilities over a network endpoint.
On the runtime side, \type{robot\_module\_kassow} consumes this endpoint and implements the \type{robot\_interface} protocol towards the rest of the system.
...
Executing the full end-to-end system requires either access to a physical Kassow robot or a compatible offline environment (e.g., REMII \cite{kassow_remii}).
