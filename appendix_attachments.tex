...
\xxx{Update this link if the maintained repository is moved/split. Optionally add a tag/commit hash if a stable release is created.}
...
\begin{description}
  ...
  \item[\path{scene/}] Example scene CAD model(s).
...
The submitted snapshot intentionally excludes development-only content (e.g., demos and retired modules) and does not redistribute third-party binaries.
\xxx{If your pruning script also removes specific internal folders (e.g., \path{demos/}, \path{default_modules_retired/}, \path{sources/}), list them explicitly here as a short bullet list.}
...
(executables, module binaries, and per-module data such as frontend \path{docroot/}).
...
\xxx{If the default port is configurable via a frontend config file, reference the exact file path (e.g., \path{.../frontend_module/module_data/config.txt}).}
...
\begin{itemize}
    ...
    These are obtained separately (typically via vcpkg) and remain under their upstream licenses.
    ...
    Their license texts are provided in \path{third\_party\_licenses/} and summarized in \path{THIRD\_PARTY\_NOTICES.md}.
\end{itemize}
...
Its original permissive notice is preserved in-file and also duplicated in \path{third\_party\_licenses/base64\_notice.txt} for convenience.
...
\path{camera/charuco\_pattern\_A4.pdf}.
For printing, the intended usage is to print at 100\% scale and mount the sheet on a flat rigid backing.
\xxx{If you used any specific print settings (paper type, lamination, etc.), add one sentence; otherwise keep this generic.}
...
\xxx{Optionally add: these values are defined in \path{diploma-thesis-code/default_modules/robot_stereo_camera_calibration_module/include/calib/charuco_defaults.h} (verify exact filename if needed).}
...
\path{camera/stereo\_camera\_robot\_mount.step}.
...
\path{pen/pen\_firmware.ino}.
The firmware is derived from the upstream D-Point project (Jcparkyn/dpoint) and modified for this thesis prototype; the upstream license text is included under \path{third\_party\_licenses/}.
\xxx{If you want to be explicit: mention that the mechanical design is only loosely inspired by D-Point, while the provided CAD is original work.}

\xxx{in appendix section subsection Pen CAD and firmware  (or whatever subsection you already have for pen artifacts), after the paragraph that lists pen.step and pen\_firmware.ino., add the following two figures with captions:}

\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{img/appendix/pen_internals_bg.png}
    \caption{Pen internal layout (CAD render).}
    \label{fig:app_pen_internals}
\end{figure}

\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{img/appendix/pen_partial_internals_bg.png}
    \caption{Pen partial internal view highlighting the electronics compartment (CAD render).}
    \label{fig:app_pen_partial_internals}
\end{figure}

\xxx{Add one referencing sentence immediately before the first figure (or immediately after the artifact listing sentence): "For reference, the internal layout is shown in \autoref{fig:app_pen_internals} and \autoref{fig:app_pen_partial_internals}."}

\xxx{Optional: if you include rendered images of the pen/mount in the attachment (recommended for readability),
add a short note and the path, e.g., \path{pen/renders/}.}

\xxx{in appendix subsection where you mention manufacturing/printing (or create a short one, e.g., subsection "3D printing layout (informative)"), right after the first sentence., add the following figure with caption:}

\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{img/appendix/pen_slice_top.png}
    \caption{3D printing plate layout (top view).}
    \label{fig:app_print_layout_top}
\end{figure}

\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{img/appendix/pen_slice_side.png}
    \caption{3D printing plate layout (perspective view).}
    \label{fig:app_print_layout_persp}
\end{figure}

\xxx{Add one referencing sentence right before the first figure: "The print plate arrangement used for manufacturing the pen parts is shown in \autoref{fig:app_print_layout_top} and \autoref{fig:app_print_layout_persp}."}
...
\xxx{Point to the exact on-disk location used by the module (e.g., configured path vs hardcoded default) once verified from \path{diploma-thesis-code/default_modules/scene_detection_stereocam_module/}.}
...
\path{scene/scene\_box.step}.

\subsection{Robot-side component (CBun) and reproducibility boundary}
\label{appendix:cbun}

The prototype uses a robot-side component (CBun) that runs on the Kassow controller and bridges the robot API to the thesis-side \type{robot\_interface} protocol.
The CBun source tree is included in the attachment under:
\path{robot/aergo\_interface\_cbun/}.
It is not part of the runtime module graph shown in \autoref{fig:runtime_full}; it is deployed to the robot controller separately.
...
\begin{itemize}
    \item Open \path{robot/aergo\_interface\_cbun/} in VS Code.
    \item Use the ``Dev Containers'' extension and ``Reopen in Container''.
    \item Configure CMake with \path{\$\{workspaceFolder\}/backend/CMakeLists.txt} and toolchain ``GCC 8.4.0''.
    \item Build the target using the provided build task (e.g., ``CBun: Clean Build'').
\end{itemize}

The resulting CBun bundle is produced at:
\path{robot/aergo\_interface\_cbun/build/aergo\_adapter.cbun}.
Deployment then consists of copying this bundle to removable media and installing it on the robot controller.
\xxx{Keep detailed robot-side installation steps in Chapter~\ref{chapter:ch5_user_workflow} (operator procedure), and keep this appendix limited to build and artifact location.}
...
\xxx{If you decide to include the REMII link in the thesis, add it here as a sentence with \url{...}.}